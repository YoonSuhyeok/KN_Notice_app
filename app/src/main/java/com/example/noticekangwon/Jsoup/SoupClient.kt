...
class SoupClient(private val AppDataBase: AppDataBase) {
    ...
    val currentTeime = Date()
    val mYear = dateformat.format ( currentTeime );
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            if (doc != null) {
                ...
                else if(fk == 20) // 건축학과
                {
                    ...
                    for (x in 5 until contents.size) {
                        ...
                        for (y in content.indices) {
                            ...
                            if (y == dateNumber) {
                                var date = content[y].text().replace('-', '.').replace('/', '.')
                                when (date.length) {
                                    5 -> date = "$mYear.$date"
                ...
                else if(fk == 27 || fk == 29)
                {
                    ...
                    var titleNumber: Int = 0
                    var dateNumber: Int = 0
                    var content = contents.select("th")
                    for (y in content.indices) {
                ...
                else if(fk == 34) // 수학교육과
                {
                    ...
                    for (x in 0 until contents.size) {
                        ...
                        var date = content[dateNumber].text().replace('-', '.').replace('/', '.')
                        when (date.length) {
                            5 -> date = "$mYear.$date"
                            8 -> date = "${mYear.substring(0..1)}$date}"
                        }
                ...
                {
                    var contents = doc.select("tr").select("[height=28]")
                    var title: String = ""; var url:String= ""; var date:String= "";
                    for( x in contents ){
                        var content = x.select("td")
    ...
    val fetchInfoList = arrayListOf<FetchMajorInfo>(
        ...
        FetchMajorInfo(74, 16, false, "https://itl.kangwon.ac.kr/ko/community/notice/list/1", 28, 0),
    )
}